...
public class searchIndex {
    public static void main(String[] args) {
        ...
        searchIndex aa=new searchIndex();

//        System.out.println(aa.search(nums,8));

        System.out.println(binarySearch2(nums,7,0,nums.length-1));
...
//            int mid = left + (right - left) / 2;
    ...
    public int searchIndex(int[] nums,int target,int begin,int end){
        ...
        if(target<nums[mid]){
            return searchIndex(nums,target,begin,mid);
        }else {
            return searchIndex(nums,target,mid,end);
        }
    }

    public static int binarySearch2(int[] nums, int target,int begin,int end) {
