 ...
 */
public class ForkJoinLongSumMainDIY {
	// 获取逻辑处理器数量 12
...
//		System.out.println("执行时间："+ Duration.between(now,Instant.now()).toMillis());
		...
		//ForkJoin计算数组总和（核心）
		ForkJoinTask<Long> result1 = fjp1.submit(ls1);

		System.out.println("forkjoin1 sum=" + result1.get());
		System.out.println("执行时间1："+ Duration.between(now1,Instant.now()).toMillis());
