...
public class ConditionTest {
    public static final Logger log = LoggerFactory.getLogger(ConditionTest.class);
...
//                lock.wait();
        ...
        new Thread(() -> {
            ...
            try {
                ...
                //注意在进行condition.signal();必须先获取锁，因为代码里面有：getExclusiveOwnerThread() == Thread.currentThread();判断
                condition.signal();
//                obj.notifyAll();
//                lock.notify();
                log.info(Thread.currentThread().getName() + " 结束处理任务");
